...
class OrtNvTensorrtrtxEngine(CudaSession):
    def __init__(
        self,
        engine_path,
        device_id,
        ...
        self.ort_nvtensorrtrtx_provider_options = self.get_nvtensorrtrtx_provider_options(
            input_profile,
            workspace_size,
            fp16,
            device_id,
        ...
        logger.info("creating NV TensorRT RTX EP session for %s", onnx_path)
        ort_session = ort.InferenceSession(
            onnx_path,
            session_options,
            providers=[
                ("NvTensorRTRTXExecutionProvider", self.ort_nvtensorrtrtx_provider_options),
            ],
        )
        logger.info("created NV TensorRT RTX EP session for %s", onnx_path)
        
        device = torch.device("cuda", device_id)
        super().__init__(ort_session, device, enable_cuda_graph)

    def get_nvtensorrtrtx_provider_options(
        self, input_profile, workspace_size, fp16, device_id, enable_cuda_graph, timing_cache_path=None
    ):
        nvtensorrtrtx_ep_options = {
            "device_id": device_id,
        }

        if enable_cuda_graph:
            nvtensorrtrtx_ep_options["nv_cuda_graph_enable"] = True

        if workspace_size > 0:
            nvtensorrtrtx_ep_options["nv_max_workspace_size"] = workspace_size
        ...
        if input_profile:
            ...
            nvtensorrtrtx_ep_options["nv_profile_min_shapes"] = ",".join(min_shapes)
            nvtensorrtrtx_ep_options["nv_profile_max_shapes"] = ",".join(max_shapes)
            nvtensorrtrtx_ep_options["nv_profile_opt_shapes"] = ",".join(opt_shapes)

        logger.info("nvtensorrtrtx_ep_options=%s", nvtensorrtrtx_ep_options)

        return nvtensorrtrtx_ep_options    

    def allocate_buffers(self, shape_dict, device):
        super().allocate_buffers(shape_dict)


class OrtNvTensorrtrtxEngineBuilder(EngineBuilder):
    def __init__(
        self,
        ...
        """
        Initializes the ONNX Runtime NV Tensorrt RTX ExecutionProvider Engine Builder.
    ...
    def build_engines(
        ...
        for model_name, model_obj in self.models.items():
            ...
            if not self.has_engine_file(engine_path):
                logger.info(
                    "Building NV Tensorrt RTX engine for %s from %s to %s. It can take a while to complete...",
                    model_name,
                    onnx_path,
                    engine_path,
                )
            else:
                logger.info("Reuse cached NV Tensorrt RTX engine in directory %s", engine_path)
